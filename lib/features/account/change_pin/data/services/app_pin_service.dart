...
class AppPinService {
  ...
  const AppPinService(this._secureStorage, this._firebaseNotificationService);
  ...
  Future<void> logoutAndClearSecurityData() async {
    try {
      ...
      if (token != null && token.isNotEmpty) {
        await _firebaseNotificationService.detachCurrentDevice(token);
      }
    } finally {
      await clearLoginToken();
      await clearSecurityData();
      _firebaseNotificationService.completeLocalLogout();
    }
  }

  Future<void> registerPushDevice() async {
    await _firebaseNotificationService.registerCurrentDevice();
  }
