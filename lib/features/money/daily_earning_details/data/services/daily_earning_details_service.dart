...
class DailyEarningDetailsService {
  final ApiConsumer apiConsumer;

  const DailyEarningDetailsService({required this.apiConsumer});

  Future<DailyMoneyDetailModel> getDaily({
    required String date,
    required String languageCode,
  }) async {
    final response = await apiConsumer.get(
      MoneyApiEndPoints.daily(date),
      await _headers(languageCode),
    );
    final body = _decode(response.body);
    if (response.statusCode == StatusCode.ok) {
      return DailyMoneyDetailModel.fromJson(body);
    }
