...
class DeductionsService {
  ...
  const DeductionsService({required this.apiConsumer});

  Future<MoneyDeductionResponse> getDeductions({
    required String languageCode,
    int page = 1,
  }) async {
    final response = await apiConsumer.get(
      MoneyApiEndPoints.deductions(page: page),
      await _headers(languageCode),
    );
    final body = _decode(response.body);
    if (response.statusCode == StatusCode.ok) {
      return MoneyDeductionResponse.fromJson(body);
    }
