...
class MyEarningService {
  final ApiConsumer apiConsumer;

  const MyEarningService({required this.apiConsumer});

  Future<EmployeeMoneyPreviewModel> getPreview({
    required String month,
    required String languageCode,
  }) async {
    final response = await apiConsumer.get(
      MoneyApiEndPoints.preview(month),
      await _headers(languageCode),
    );
    final body = _decode(response.body);
    if (response.statusCode == StatusCode.ok) {
      return EmployeeMoneyPreviewModel.fromJson(body);
    }
    throw ServerException(serverFailure: ServerFailure.fromJson(body));
  }

  Future<MoneyTrendModel> getTrend({
    required String period,
    required String month,
    required String languageCode,
  }) async {
    final response = await apiConsumer.get(
      MoneyApiEndPoints.trend(period: period, month: month),
      await _headers(languageCode),
    );
    final body = _decode(response.body);
    if (response.statusCode == StatusCode.ok) {
      return MoneyTrendModel.fromJson(body);
    }
    throw ServerException(serverFailure: ServerFailure.fromJson(body));
  }
