...
class PayslipsService {
  ...
  const PayslipsService({required this.apiConsumer});

  Future<MoneyPayslipsResponse> getPayslips({
    required int page,
    required String languageCode,
  }) async {
    final response = await apiConsumer.get(
      MoneyApiEndPoints.payslips(page: page),
      await _headers(languageCode),
    );
    final body = _decode(response.body);
    if (response.statusCode == StatusCode.ok) {
      return MoneyPayslipsResponse.fromJson(body);
    }
